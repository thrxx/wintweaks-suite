...
:: v2.4.0: Исправление закрытия консоли, безопасная очистка логов, корректная работа
::          CleanTaskbar/CleanStartMenu для Win10/Win11, обработка ошибок в SystemCleanup
:: v2.5.0: [КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ] Устранена потеря переменных в :CheckStatus
::          (убран setlocal из парсера, возвращен прямой запрос с нормализацией).
::          Исправлен поток ApplyAll (добавлен флаг BATCH_MODE для предотвращения
::          преждевременного goto menu). Оптимизирован перезапуск Explorer
::          (единственный вызов в конце массовых операций). Интегрированы
::          стабильные решения из v1.11 с сохранением архитектуры v2.x.
:: ============================================================================
...
:: Сброс флага массового применения
set "BATCH_MODE=0"
call :CheckStatus
goto menu
...
:: ============================================================================
:: v2.5.0: ПРЯМОЙ ПАРСИНГ РЕЕСТРА БЕЗ setlocal (ФИКС ПОТЕРИ ПЕРЕМЕННЫХ)
:: ============================================================================
:CheckStatus
:: v2.5.0: Убран вызов :GetRegVal. Прямой запрос предотвращает потерю переменных
::          из-за изоляции setlocal/endlocal в подпрограммах.
set "TMP="
...
if !errorlevel! equ 0 (
    ...
    :: v2.3.0: Обновление политик для HKLM
...
:: ============================================================================
:: v2.5.0: БЕЗОПАСНЫЙ ПЕРЕЗАПУСК EXPLORER (ОТЛОЖЕННЫЙ И ДЕДУПЛИЦИРОВАННЫЙ)
:: ============================================================================
...
:: ============================================================================
:: ФУНКЦИИ ПРИМЕНЕНИЯ (v2.5.0: ИСПРАВЛЕН ПОТОК ВЫПОЛНЕНИЯ)
:: ============================================================================
:ApplyPowerPlan
echo.
...
:: v2.5.0: В пакетном режиме откладываем перезапуск, чтобы не убить консоль mid-flow
if "%BATCH_MODE%"=="1" (
    echo [*] Перезапуск Explorer отложен до завершения ApplyAll...
    goto :eof
)
...
:: ============================================================================
:: v2.5.0: ПАНЕЛЬ ЗАДАЧ / ПУСК / ОЧИСТКА
:: ============================================================================
:CleanTaskbar
cls
echo.
...
:: v2.5.0: Только для Win10. Win11 игнорирует COM из-за XAML.
if "%OS_TYPE%"=="win10" (
    ...
    echo [4/4] Win11: ручное открепление через Параметры -> Персонализация
)
...
echo [3/5] Очистка кэша обновлений...
...
echo [5/5] DISM очистка...
dism /online /cleanup-image /startcomponentcleanup /resetbase >nul 2>&1
echo [+] DISM завершена

echo.
echo [!] Очистка логов пропущена (compliance risk + зависания)
echo [+] Корзина очищена (PowerShell Clear-RecycleBin)
...
:: ============================================================================
:: v2.5.0: APPLYALL / RESTORE (ПОСЛЕДОВАТЕЛЬНЫЙ ПОТОК + ЕДИНЫЙ RESTART)
:: ============================================================================
:ApplyAll
cls
echo %green%[*] ПРИМЕНЕНИЕ ВСЕХ НАСТРОЕК...%reset%
echo ================================================================================
:: v2.5.0: Включаем пакетный режим, чтобы функции не прыгали в menu и не убивали консоль
set "BATCH_MODE=1"
call :ApplyPowerPlan
call :ApplyUWP
call :ApplyDelivery
...
:: v2.5.0: Единый безопасный перезапуск в конце
call :RestartExplorerGracefully
set "BATCH_MODE=0"
...
call :CleanStartMenu
:: v2.5.0: Единый перезапуск вместо дублирующих вызовов
call :RestartExplorerGracefully
echo.
echo ================================================================================
